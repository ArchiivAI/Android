package com.example.archivai.domain.repository.home

import com.example.archivai.data.source.remote.responseModels.home.GetDocsResponseModel
import com.example.archivai.data.source.remote.responseModels.home.QuickSection
import com.example.archivai.domain.entities.QuickAccessSection
import com.example.archivai.domain.entities.Section
import com.example.archivai.domain.entities.StorageDoc
import com.example.archivai.domain.entities.StorageUsed
import com.example.archivai.domain.entities.UserDetails

interface HomeRepository {
    suspend fun getStorageUsed() : StorageUsed
    suspend fun getDocs() : StorageDoc
    suspend fun getActivityLog()
    suspend fun getUserDetails(): UserDetails
    suspend fun getQuickAccessSections(): List<QuickAccessSection>

}