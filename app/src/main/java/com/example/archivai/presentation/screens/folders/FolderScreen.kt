...
fun FoldersScreen(
    ...
    Box(
        ...
        if (state.isFabBottomSheetVisible) {
            FolderFabBottomSheet(
                ...
                onUploadFileClick = {
                    ...
                    viewModel.showFileOptionsBottomSheet()
                },
        ...
        if (state.isFileOptionsBottomSheetVisible) {
            val filePicker = rememberFilePicker { files ->
                viewModel.addSelectedFiles(files)
                viewModel.hideFileOptionsBottomSheet()
                if (files.isNotEmpty()) {
                    viewModel.showUploadConfirmationBottomSheet()
                    viewModel.disablePhotoMode()
                }
            }
            ...
            Box(
                ...
                UploadFileOptionsBottomSheet(
                    onDismissRequest = { viewModel.hideFileOptionsBottomSheet() },
                    onThisDeviceClick = { filePicker() },
                    onTakePhotoClick = { cameraCapture() }
                )
            }
        }

        if (state.isUploadConfirmationBottomSheetVisible) {
            Box(
                modifier = Modifier
                    .fillMaxSize()
                    .background(Color.Black.copy(alpha = 0.5f))
            ) {
                UploadFileConfirmationBottomSheet(
                    viewModel = viewModel,
                    onDismissRequest = { viewModel.hideUploadConfirmationBottomSheet() },
