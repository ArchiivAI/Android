...
fun ForgetPasswordScreen(navController: NavController , viewModel : ForgetPasswordViewModel = hiltViewModel()) {
    ...
    Column(
        ...
        Button(
            onClick = { viewModel.sendChangePasswordMail(email) },
            modifier = Modifier.fillMaxWidth().height(48.dp).padding(start = 16.dp, end = 16.dp),
            colors = ButtonDefaults.buttonColors(containerColor = Color(0XFF132863))
        ) {
            if (state is ForgetPasswordUiState.Loading) {
